...
export function TitleBar(): JSX.Element {
  ...
  return (
    <div
      ...
      data-titlebar-tabs={hostingTabs ? '' : undefined}
    >
      {hostingTabs ? (
        <>
          <div className="flex shrink-0 items-center gap-1">
            <button
              type="button"
              aria-label={sidebarOpen ? 'Hide sidebar (⌘1)' : 'Show sidebar (⌘1)'}
              title={sidebarOpen ? 'Hide sidebar (⌘1)' : 'Show sidebar (⌘1)'}
              onClick={toggleSidebar}
              className="no-drag flex h-8 w-10 items-center justify-center rounded-md text-ink-600 hover:bg-paper-200"
            >
              <PanelLeftIcon width={16} height={16} />
            </button>
          </div>
          <div id="titlebar-tabs-host" className="min-w-0 flex-1 self-stretch" />
          {!isMac && (
            <div className="flex shrink-0 items-center gap-1">
