...
export const HELP_SETTINGS: HelpSettingsSection[] = [
  ...
  },
  {
    title: 'Templates',
    items: [
      { label: 'Template library', detail: 'Browse every template — built-in and custom. Built-ins cover engineering (ADR, RFC, Bug Report, Postmortem, Meeting Notes, 1:1) and personal use (Reading Notes, Journal, Project Kickoff, To-do).' },
      ...
      { label: 'Where templates appear', detail: 'Use a template via the picker (`Space t` / `:template` / “New Note from Template…”), or from a folder’s right-click “New from template”. Custom templates require a local vault; built-ins work everywhere.' }
    ]
  },
  {
    title: 'CLI',
...
export const HELP_CLI: HelpCard[] = [
  ...
  },
  {
    ...
    body:
      'Prefer `zen append` and `zen prepend` over `zen write` for journals and running lists — they preserve the rest of the body. Both accept `--body "literal"` or `--body -` to read stdin (so `cat ideas.txt | zen append journal.md --body -` works).'
  },
  {
